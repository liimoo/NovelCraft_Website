...
- **文体**: です・ます調（丁寧語）で統一する
- **文字数**: 400字程度を目安にする
- **画像**: 必ず1枚以上添付する（スクリーンショット・素材画像など）
  - ユーザーからスクショを受け取った場合 → REST APIでメディアアップロードしてアイキャッチに設定
  - スクショがない場合 → `novel-assets/bg/` や `novel-assets/characters/` の既存画像をアップロードして代用
  - アップロード: `POST /wp-json/wp/v2/media`、アイキャッチ設定: `POST /wp-json/wp/v2/posts/{id}` に `featured_media` を指定
